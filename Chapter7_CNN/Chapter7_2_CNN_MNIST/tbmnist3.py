...
def build_model(img_shape: Tuple[int, int, int], num_classes: int) -> Sequential:
    ...
    model.add(Conv2D(filters=16, kernel_size=3, padding="same", input_shape=img_shape))     # nunr input layer hat input_shape
    model.add(Activation("relu"))                       #  muss hier checken welcher shape rauskommt                         # default values: pool_size= (2,2), strides=2, padding="valid"                                                          # man muss hier auf alle fälle behirnen dass die poolsize und stride zusammen passen sollten                                                    # man muss hier auch behirnen das nach dem Maxpooling sich die grösse der Bildmatrix wieder ändert
    model.add(Conv2D(filters=16, kernel_size=3, padding="same", input_shape=img_shape))
